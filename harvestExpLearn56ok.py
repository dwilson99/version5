...
def run_harvest_orig():
	print("33 Harvester v 4.0 w/mods")
	print("34 start run_harvest_orig()")
	...
	print("55 after extract_features() trainingSets :", trainingSets)
	...
	timeEnd = time.time()
	print("\n72 Total inno–clean_pc() processing time = ", round((time_inno), 3)  )
	print("\n73 Total harvest processing time = ", round((timeEnd-timeStart_1), 1)	)
	print("74 Reached the end of harvest...")
...
#save raw classification data
	...
	for trainingSet in trainingSets:
		#print("545 trainingSet: ", trainingSet)
		...
		for traininglabel in trainingLabels:
			#print('573 trainingLabel: ', traininglabel)
...
#				  print(memorySpec)
				...
				otherDoc = [numberListElements]

				i = 0
				for otherDocName in otherLabels:
					otherDocName = 'otherDoc' + '_' + str(i)
					print('33 otherDocName', otherDocName)
					i = i + 1
				...
				for otherLabel in otherLabels:
					print('\n 24 index, : ', str(index), '\t otherLabel: \t', otherLabel)
					file_name = otherLabel+"_spec.txt"
					print('26 file_name:', file_name)
					fle = open(file_name)
					print('28', fle )
					otherDocName = 'otherDoc' + '_' + str(index)
					otherDocName = json.load(fle)
					fle.close()
					otherDocNameSamples = float(otherDocName['samples'])
					print('31 otherDocNameSamples', otherDocNameSamples)
					index = index + 1
					...
					#		print('46 otherDocName	= ', otherDocName)
					#		print('48 ', otherDocNameSamples)
							if term in otherDocName:
								(odf,otf) = otherDocName[term]
								df += odf
					#		print('\n52 index2: ', index2)
					#		print('53 otherDocName: ', otherDocName)
					#		print('54 ', otherDocNameSamples)
							otherSample = otherSample + float(otherDocNameSamples)
					#		print('56 ', otherSample)
							index2 = index2 + 1
						tfIdf = tf * (math.log(((1.5+otherSample)/(1+df))))
					#	print('59 tf: ', tf, '\ttfIdf: ', tfIdf)
					#	print('60 term: ', term)
						memorySpec[term] = (tf,tfIdf)
